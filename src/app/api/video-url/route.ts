...
export async function POST(request: Request) {
  try {
    ...
    let key = match.videoKey;

    if (clipId) {
      const clip = Array.isArray(match.clips)
        ? match.clips.find((item: { id?: string }) => item.id === clipId)
        : null;
    ...
    return Response.json(
      { url: await signedVideoUrl(key), expiresIn: 60 * 60 * 2 },
